...
6. 如果你的 skill 是用户入口 wrapper，优先让它调用 `invoker bootstrap <skill> --json`，而不是自己拼接复杂的 readiness 状态机
7. 只有在需要兼容 legacy 输入或补充宿主私有信息时，再使用 `invoker.skill.yaml`
8. 如果仓库里同时保留 `SKILL.md` 与 `skill.yaml`，Invoker 会以 `SKILL.md` 为准，并给出兼容告警

这样 `bootstrap/scan/doctor/install/fix/run` 就能形成闭环。
